...
- **TypeScript**: Run `pnpm typecheck` (tsc --noEmit on both tsconfig.json and tsconfig.node.json)
- **Vite**: Run `pnpm build` (tsc -b + vite build)
- **Biome**: Run `pnpm biome:check` for lint + format validation
- **ESLint**: Run `pnpm lint` for additional linting
- Always read package.json scripts for available commands
